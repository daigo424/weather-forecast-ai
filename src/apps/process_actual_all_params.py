"""
data/01_raw の actual JSON を読み込み、data/02_processed に月単位 Parquet として保存する。
  input:  data/01_raw/open-meteo/actual/location=<name>/year=<Y>/month=<MM>/day=<DD>/raw_actual.json
  output: data/02_processed/open-meteo/actual/location=<name>/year=<Y>/month=<MM>/data_actual.parquet
"""
...
logger = AppLogger("process_actual")
...
def _processed_month_path(location_name: str, year: int, month: int) -> Path:
    return (
        PROCESSED_DIR / "open-meteo" / "actual"
        / f"location={location_name}"
        / f"year={year}"
        / f"month={month:02d}"
        / "data_actual.parquet"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process actual raw JSON to Parquet in data/02_processed")
